...
<head>
    ...
    <style>
        ...
        .s_table-container {
            max-height: 89%; /* Adjust scroll height */
        ...
        .s_table th {
            background-color: #338b33; /* Your preferred color */
            color: white;
            position: sticky;
            top: 0;
            z-index: 1; /* Ensure header stays above scrollable content */
            font-size: 15px;
        }
...
//orders history
function viewHistory(cid,psno,name,fd,td,tamt,piamt,peamt){
    ...
    $.ajax({
        ...
        success:function(response){
            ...
            if(response.data.length > 0){
                ...
                <td>${dt.Email}</td>
                <td>${dt.Phone2}</td>
                <td class="p_fromdate">${dt.from_date}</td>
                <td class="p_todate">${dt.to_date}</td>
                ...
                <td><button onclick="payment_update(event)" class="payment_update_btn" disabled>Edit</button></td>    
                <td><button onclick="viewHistory('${dt.customer_id}', '${dt.sno}', '${dt.CustomerName}', 
                '${dt.from_date}', '${dt.to_date}', ${dt.total_amount}, ${dt.paid_amount}, ${dt.unpaid_amount})">View</button></td>    
                `
...
function payment_update(event){
    ...
    if(previous_paid_amount === new_value){
       ...
       var payload = {
         ...
         fromdate:ptrow.querySelector('.p_fromdate').textContent,
         todate:ptrow.querySelector('.p_todate').textContent,
         load:"update_payment",
